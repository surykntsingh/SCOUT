...
class Trainer:
    ...
    def train(self, model, datamodule, fast_dev_run=False):
        ...
        early_stop_callback = EarlyStopping(monitor="val_loss", min_delta=1e-5, patience=21, verbose=True, mode="min")
...
class KFoldTrainer(Trainer):
    ...
    def train_and_test(self, fast_dev_run=False):
        ...
        for fold, (train_idx, test_idx) in enumerate(self.__kf.split(files)):
            ...
            early_stop_callback = EarlyStopping(monitor="val_loss", min_delta=1e-5, patience=7, verbose=True,
                                                mode="min")
            trainer = pl.Trainer(
                max_epochs=self.max_epochs,
                callbacks=[checkpoint_callback, early_stop_callback],

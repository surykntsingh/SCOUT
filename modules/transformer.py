...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        print(f'nbatches: {nbatches} query: {query.shape}, key: {key.shape}, value: {value.shape}')
        query, key, value = [
...
class EncoderDecoder(AttModel):
    ...
    def __build_model(self, tgt_vocab):
        ...
        # pgc = PositionalEncoding(tgt_vocab, self.dropout)
        pp = PAM(self.d_model)
        model = Transformer(
            ...
            nn.Sequential(nn.Linear(self.d_model, 256), nn.ReLU(), nn.Linear(256, self.d_model), deepcopy(position))
        )
        return model
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks,gc_emb, meshes=None):
        att_feats = pad_tokens(att_feats)
        att_feats, gc_feats, seq, _, att_masks, seq_mask, _ = self._prepare_feature_forward(att_feats, gc_emb, att_masks, meshes)
